    ...
    static auto layout = Container::Vertical(
        {
            ...
            Container::Horizontal({appModeLabel,
                                   Dropdown(&_dispModel.appModeEntries, &_dispModel.appMode), Button("Save Talemetry", [&]()
                                                                                                     { _onSaveTelemetryClicked(); })}),
...
void TuiSystem::_onSaveTelemetryClicked()
{
    _dispModel.world->emit<OnSaveTelemetryRequest>(OnSaveTelemetryRequest());
}
